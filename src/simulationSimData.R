...
out3 <- vector(mode = "list", length = Nn)
...
for (i in 1:Nn){
  ...
  sizelist3 <- vector(mode = "list", length = NN)
  for (j in 1:NN){                                                                                ## For each value of the sample size
    ...
    for (k in 1:NLbd){                                                                            ## For each value of the lambda parameter
      ...
      adhocEstim3 <- array(0, dim = c(Hvec[i], NEst, NFreq))
      for (cnt in 1:NFreq){
        for (l in 1:NEst){
          ...
          adhocEstim3[,l,cnt] <- tmp[[3]]                                                        ## Ad hoc estimates for frequencies
        }
      }
      lbdalist[[k]]  <- Estim
      lbdalist1[[k]] <- adhocEstim1
      lbdalist2[[k]] <- adhocEstim2
      lbdalist3[[k]] <- adhocEstim3
    }
    sizelist[[j]]  <- lbdalist
    sizelist1[[j]] <- lbdalist1
    sizelist2[[j]] <- lbdalist2
    sizelist3[[j]] <- lbdalist3
  }
  out[[i]]  <- sizelist
  out1[[i]] <- sizelist1
  out2[[i]] <- sizelist2
  ...
  saveRDS(out3, file = paste0(path, "dataset/adhocModelEstimates3.rds"))
